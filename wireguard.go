...
// setupWireGuard creates and configures a WireGuard interface inside the current network namespace.
// Must be called from within an ns.Do() closure.
func setupWireGuard(ifName string, conf *Config) error {
	...
	addr, err := parseAddress(conf.Address)
	if err != nil {
	...
	if err := configureWireGuard(ifName, conf); err != nil {
		_ = netlink.LinkDel(createdLink)
		return err
	}
	...
	if err := addPeerRoutes(createdLink, conf.Peers); err != nil {
		_ = netlink.LinkDel(createdLink)
		return err
	}
...
// configureWireGuard opens a wgctrl client and applies the WireGuard configuration.
// Must be called from within an ns.Do() closure.
func configureWireGuard(ifName string, conf *Config) error {
	...
	privateKey, err := parseWGKey(conf.PrivateKey)
	if err != nil {
...
// teardownWireGuard removes the WireGuard interface. Idempotent: not-found is not an error.
// Must be called from within an ns.Do() closure.
...
// checkWireGuard verifies that the WireGuard interface exists and matches the configuration.
// Must be called from within an ns.Do() closure.
func checkWireGuard(ifName string, conf *Config) error {
	link, err := netlink.LinkByName(ifName)
	if err != nil {
	...
	expectedAddr, err := parseAddress(conf.Address)
	if err != nil {
		return err
	}
...
// parseWGKey decodes a base64-encoded WireGuard key.
