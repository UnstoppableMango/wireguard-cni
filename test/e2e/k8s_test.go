	...
	for _, ver := range testutils.AllSpecVersions {
		Describe(fmt.Sprintf("cni %s", ver), Label(ver), Ordered, func() {
			var (
				serverConf      []byte
				clientConf      []byte
				serverAddResult []byte
				clientAddResult []byte
			)
			...
			BeforeAll(func(ctx context.Context) {
				...
				serverConf, err = server.CniConfig(ver, wgPort, []utils.CNIPeer{
					client.ClientPeer(),
				})
				Expect(err).NotTo(HaveOccurred())

				clientConf, err = client.CniConfig(ver, 0, []utils.CNIPeer{
